...
#[test]
fn test_near_zero() {
    assert!(modern_robotics::utils::near_zero(1e-7));
    assert!(modern_robotics::utils::near_zero(0.0));
    assert!(!modern_robotics::utils::near_zero(1e-5));
    assert!(!modern_robotics::utils::near_zero(1.0));
}

#[test]
fn test_normalize() {
    let v = nalgebra::DVector::from_vec(vec![1.0, 2.0, 3.0]);
    let norm_v = modern_robotics::utils::normalize(v);
